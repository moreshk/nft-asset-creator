...
for i in range(2):
    ...
    character_species = random.choices(species, weights=species_weights, k=1)[0]
    character_class = random.choices(classes, weights=class_weights, k=1)[0]
    character_background = random.choices(backgrounds, weights=background_weights, k=1)[0]
    character_alignment = random.choices(alignments, weights=alignment_weights, k=1)[0]
    ...
    character_sex = random.choice(sexes)
        # Randomly select a gold value between 5 and 65, in increments of 5
    character_gold = random.randrange(5, 70, 5)
    # Read the trinkets from a file and store them in a list
    with open('trinkets.txt', 'r') as file:
        trinkets = [line.strip() for line in file]

    # Randomly select one trinket
    character_trinket = random.choice(trinkets)
    ...
    'Blue', 'Green', 'Purple', 'Pink', 'Orange', 'Silver'
    ]
    ...
    skin_colours = [
    'Ivory', 'Olive', 'Tan', 'Bronze', 'Amber',
    'Caramel', 'Sienna', 'Chestnut', 'Mahogany', 'Ebony',
    'Alabaster', 'Porcelain', 'Onyx'
    ]
    ...
    image_backgrounds = [
    'Woods', 'Mountains', 'Volcanic', 'Underworld', 'Ruins',
    'Plains', 'Arctic Snow', 'Coastal', 'Cosmic', 'Desert',
    'Castle', 'Town centre'
    ]

    image_background_weights = [
        11, 11, 4, 4, 10,
        11, 10, 11, 2, 10,
        8, 8
    ]

    character_hair_colour = random.choices(hair_colours, weights=hair_colour_weights, k=1)[0]
    ...
    character_skin_colour = random.choices(skin_colours, weights=skin_colour_weights, k=1)[0]
    character_image_background = random.choices(image_backgrounds, weights=image_background_weights, k=1)[0]
    ...
    prompt = f"dungeons and dragons {pfp_traits['Sex'].lower()} {character_traits['Species'].lower()} character, " \
            ...
            f"close up portrait, " \
            f"{pfp_traits['Hair Colour'].lower()} hair, " \
            f"{pfp_traits['Skin Colour'].lower()} skin, wearing {pfp_traits['Clothing'].lower()} and " \
            f"wearing a {pfp_traits['Headpiece'].lower()} on the head." \
            f"character profession: {character_traits['Background'].lower()} , " \
